...
function update_inventarios_simple(el) {
    var data = {
        item_id: $(el).closest("li").data().id,
        store_id: store_id,
        units: $(el).closest("li").find(".utotal").val(),
        options: ""
    };

    call_api('/api/items/set_inventory', 'post', data, function(res) {
    });
}
...
$(document).ready(function() {
    ...
    $(".edit-attribute").change(function() {
        if ($(".edit-attribute").val() == "live") {
            ...
            $("#sortable_items").hide();
            $("#inventory").show();
            if ($("#inventory").find("li").length < 1) {

                $.ajax({
                    type: "POST",
                    beforeSend: function() {
                        $("#inventory").html("Cargando inventario...");
                    },
                    url: '/api/items/get_inventory',
                    ...
                    success: function(data) {
                        ...
                        for (var n in items) {
                            ...
                            $(li).append("<div class='mopt col-sm-6'><b>Total de unidades:</b> <input onblur='update_inventarios_simple(this)' class='utotal form-control' type='text' " + (items[n].options ? "disabled" : "") + " value='" + items[n].units_availible + "'/></div></div>");
                                ...
                                for (var v in options) {
                                    ...
                                    $(input).blur(function() {
                                        if (isNaN($(this).val())) {
                                            ...
                                            var opts = new Array();
                                            $(this).closest("li").find(".opt-container").each(function(j, f) {
                                                var obj = {
                                                    label: $(f).data().label,
                                            ...
                                            var data = {
                                                ...
                                                store_id: store_id,
                                                units: $(this).closest("li").find(".utotal").val(),
