...
function publish(event) {
    ...
    call_api('/api/stores/solicitud_publicacion', 'post', {store_id: store_id}, function(data) {
        $("#preloader").data().Preloader.methods.hide();
    }, null, function() {
        $("#preloader").data().Preloader.methods.setText("" + kontrol_lang['layout_sending_publication_request'] + "");
...
function add_store(event) {
    event.preventDefault();
    event.stopPropagation();
    call_api('/api/stores/add_store', 'post', {'store_id': store_id}, function(r) {
        var data = jQuery.parseJSON(r.toString());
        if (data.data['3rrC'] == undefined) {
            window.location = "/stores/id/" + data.data;
        } else {
            $("#preloader").data().Preloader.methods.draw([{issue: "Error al crear la tienda"}], "error");
            $("#preloader").data().Preloader.methods.show();
        }
    ...
    function search(val) {
        call_api('/api/users/get_user_stores', 'post', {search: val}, function(data) {
            ...
            try {
                ...
                $("#my-stores").append('<li role="presentation" class="dropdown-header"><a href="#" onclick="return add_store(event);" class="btn btn-default pull-right">' + kontrol_lang['layout_add_store'] + '</a></li>');
...
function doTree(tree, plugins, update, multiple) {
...
function delete_items(store_id, tobedeleted) {
    call_api('/api/items/update_items_properties', 'post', {store_id: store_id, json: JSON.stringify(tobedeleted)}, function(data) {
        ...
        $("#preloader").data().Preloader.methods.show();*/
        window.location.href="/stores/id/"+store_id;
    });
}
